...
| Tag | Meaning |
| --- | --- |
| `1.0.19` (current) | Pinned release tag |
...
The version lives in `package.json` (`1.0.19`). After bumping it, run:
...
2. Tag the release commit: `git tag -a v1.0.19 -m "..."` and `git push origin v1.0.19`.
3. CI builds and pushes `ghcr.io/devosduotech/dashhub:1.0.19` + `:latest`.
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t ghcr.io/devosduotech/dashhub:1.0.19 \
